...
import '../styles/cadastro.css'; // Reusa o estilo do formulário
import '../styles/dashboard.css'; // Reusa estilos de lista
...
const ListaParticipantes = () => {
  ...
  const initialSearch = searchParams.get('q') || ''; // Pega o termo da URL se vier do Dashboard
  ...
  useEffect(() => {
    ...
    const fetchTodos = async () => {
      ...
      try {
        // Reusa a rota que criamos para o CSV
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', color: '#fff' }}>
                    <thead>
                        <tr style={{ borderBottom: '2px solid #FEBF00', textAlign: 'left' }}>
                            ...
                            <th style={{ padding: '10px' }}>Idade/Nasc.</th>
                    ...
                    <tbody>
                        {participantesFiltrados.map((p) => (
                            <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                                ...
                                <td style={{ padding: '10px' }}>{p.dataNascimento}</td>
                                <td style={{ padding: '10px' }}>
                                    <button 
                                        onClick={() => navigate(`/perfil/${p.id}`)}
                                        style={{ 
                                            background: '#FEBF00', color: '#5c0017', border: 'none', 
                                            borderRadius: '5px', padding: '5px 10px', cursor: 'pointer', fontWeight: 'bold' 
                                        }}
                                    >
                                        Ver Perfil
                                    </button>
